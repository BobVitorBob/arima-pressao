...
def plot(vals, labels=None, detected_anomalies=[], continuous_anomalies=[], margins=None, title='', save=False, img_name=None, show=True, divisions=[], sec_plots=[], std_dev=None):
	X=range(len(vals))
	...
	if margins:
		for margin in margins:
			plt.fill_between(range(len(vals)), margin[0], margin[1], alpha = 0.1, color="g")
	if std_dev:
		u_b = []
		l_b = []
		for val, std in zip(vals, std_dev):
			u_b.append(val + std)
			l_b.append(val - std)
		plt.fill_between(range(len(std_dev)), u_b, l_b, alpha = 0.1, color="b")
	fig.tight_layout()
	if save:
